...
export async function createServer(
  ...
  toolHandlerRegistry.register("store_agent_review_draft", "Save a reviewing agent's draft without posting to GitHub", { type: "object", properties: { sessionId: { type: "string" }, agentId: { type: "string" }, aiTool: { type: "string", enum: ["claude", "gemini", "codex"] }, runId: { type: "string" }, model: { type: "string" }, verdict: { type: "string", enum: ["APPROVE", "REQUEST_CHANGES", "COMMENT"] }, body: { type: "string" }, comments: { type: "array" } }, required: ["sessionId", "agentId", "aiTool", "verdict", "body"] }, async (args) => {
    try {
      ...
      const draft = await reviewSessionDraftsRepo.upsertDraft({ sessionId, agentId: args.agentId as string, aiTool: args.aiTool as string, runId: (args.runId as string) ?? null, model: (args.model as string) ?? null, verdict: args.verdict as string, body: args.body as string, commentsJson: JSON.stringify(comments) });
